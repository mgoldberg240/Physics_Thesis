...
def Y(L_max,m,s,cos_theta):
    ...
    init=jacobi.envelope(a,b,0,0,cos_theta)
...
def xi(L_max,m,s,format='dia'):
    """ needed for the ball """
    
    l = np.arange(L_min(m,s),L_max+1)

    sin,cos = np.sqrt(l/(2*l+1)), np.sqrt((l+1)/(2*l+1))

    sin = sparse.dia_matrix((sin,[0]),shape=(len(l),len(l)))
    cos = sparse.dia_matrix((cos,[0]),shape=(len(l),len(l)))

    if format == 'dia': return sin, cos
    return sparse.csr_matrix(sin), sparse.csr_matrix(cos)
