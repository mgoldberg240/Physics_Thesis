...
Om = 2*np.pi
tau = 25
Amp = 1e-5  # initial noise amplitude
Gamma = 0.6*2*np.pi
Reynolds = 1e3
nu = Gamma/Reynolds

# Integration parameters
dt = 0.01*2*np.pi/(L_max+1)  # timestep
...
P,M,L = [],[],[]
for m in range(m_start,m_end+1):
    Mm,Lm = eq.advection(S,m,[2*Om,nu,tau])
...
def nonlinear(state_vector,RHS):
    ...
    F_ph['g'] = -(v_th['g']*v_phth['g'] + v_ph['g']*v_phph['g']) + u_jet['g']/tau
